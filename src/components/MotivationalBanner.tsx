...
export const MotivationalBanner = ({
  ...
  return <Card className="mb-4 border-success/60">
      <div className="p-4 space-y-3">
        {/* Motivational Quote */}
        <div className="bg-success/20 rounded-lg p-3 flex items-center gap-3 transition-all duration-500">
          <span className="text-2xl sm:text-3xl">{currentQuote.icon}</span>
          <div className="flex-1">
            <p className="font-semibold text-sm sm:text-lg text-success-foreground">{currentQuote.text}</p>
          </div>
          <Sparkles className="w-5 h-5 text-success-foreground animate-pulse" />
        </div>
        
        {/* Days Without Smoking */}
        <div className="text-center bg-success/20 rounded-lg p-3">
          <h2 className="text-sm sm:text-lg font-medium text-muted-foreground mb-2">
            {t('dashboard.daysWithoutSmoking')}
          </h2>
          <p className="text-3xl sm:text-5xl font-bold text-success">{daysWithoutSmoking}</p>
        </div>

        {/* Lifehack of the Day */}
        {currentTip && currentTip.description && (
          <div className="bg-success/20 rounded-lg p-3">
            <h3 className="text-xs sm:text-sm font-semibold text-success mb-2">
