  ...
  const handleNext = async () => {
    if (step < 5) {
      ...
      try {
        ...
        const {
          error: planError
        } = await supabase.from("smoking_plans").insert({
          user_id: user?.id,
        ...
        <CardContent>
          <div className="flex justify-center mb-8">
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map(i => <div key={i} className={cn("h-2 w-12 rounded-full transition-colors", i <= step ? "bg-primary" : "bg-muted")} />)}
            </div>
          </div>
          ...
          handleNext();
            ...
            {step === 1 && <div className="space-y-4">
                ...
                <Input id="cigarettesPerDay" type="number" min="1" placeholder="Например: 20" value={formData.cigarettesPerDay} onChange={e => setFormData({
              ...formData,
            ...
            {step === 2 && <div className="space-y-4">
                ...
                <Input id="packPrice" type="number" min="0" step="0.01" placeholder="Например: 200" value={formData.packPrice} onChange={e => setFormData({
              ...formData,
            ...
            {step === 3 && <div className="space-y-4">
                ...
                <Input id="minutesPerBreak" type="number" min="1" placeholder="Например: 5" value={formData.minutesPerBreak} onChange={e => setFormData({
              ...formData,
            ...
            {step === 4 && <div className="space-y-4">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !formData.quitDate && "text-muted-foreground")}>
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {formData.quitDate ? format(formData.quitDate, "PPP", {
                    locale: ru
                  }) : <span>Выберите дату</span>}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0">
                    <Calendar mode="single" selected={formData.quitDate} onSelect={date => setFormData({
                  ...formData,
                  quitDate: date
                })} initialFocus locale={ru} />
                  </PopoverContent>
                </Popover>
              </div>}
            ...
            {step === 5 && <div className="space-y-4">
                ...
                <Input id="reductionPerWeek" type="number" min="1" placeholder="Например: 2" value={formData.reductionPerWeek} onChange={e => setFormData({
              ...formData,
            ...
            <div className="flex gap-4">
              {step > 1 && <Button type="button" variant="outline" onClick={handleBack} className="flex-1">
                  Назад
                </Button>}
